...
export default SortableGroupComponent.extend({
  ...
  */
  ...
  inspectClass(className, element, property){
    var inspector = $("<div>").css('display', 'none').addClass(className);
    $(element).append(inspector); // add to DOM, in order to read the CSS property

    let result = $(inspector).css(property);
    $(inspector).remove();
  ...
  heightChangedAmount: 0,

  setHeightChangedAmount(amount){
    this.set('heightChangedAmount', amount);
  },
  ...
  hasChild(item, key, operand, value, thisoperator=null)
  {
    ...
    if(children && children.length > 0)
    {
      ...
      children.forEach(child => {
        ...
        {
          //recursive children
          hasChildCount = hasChildCount + this.hasChild(child, key, operand, value);
        }
      });
    }
...
*/
  ...
  isSwap(){
    ...
    if(this.dropTarget && (draggedComponentParent.get('elementId') !== this.dropTarget.get('elementId')))
    {
      ...
      //update the height of the dragged item to reflect any styling differences between root and subfolders (eg. margin-bottom)
      //what is the margin-bottom inside this folder?
      let className = $(this.currentlyDraggedComponent.element).attr('class');
      let element = this.dropTarget.element;

      let margin = parseFloat(this.inspectClass(className, element, 'margin-bottom'));

      this.currentlyDraggedComponent.set('swappedDestinationHeight', $(this.currentlyDraggedComponent.get('element')).outerHeight() + margin);
    }
...
COPY:
    ...
    */
     ...
     */
     ...
     }, this.cssTransitionDuration * 2); //125ms timeout, this should be the same as your CSS transitions
    ...
    //console.log((item.get('parent') ? '  :nested>'+item.get('parent.elementId')+' ': '') + index + item.get('elementId') + " isDragging="+item.isDragging+" position="+position+" draggedBottomEdge="+draggedBottomEdge+" draggedTopEdge="+draggedTopEdge+" item.get('topEdge')"+item.get('topEdge')+" item.get('bottomEdge')"+item.get('bottomEdge')+(prevItem ? " prevItem.get('topEdge')="+prevItem.get('topEdge')+" prevItem.get('bottomEdge')="+prevItem.get('bottomEdge') : 'prevItem=false')+(nextItem ? " nextItem.get('topEdge')="+nextItem.get('topEdge')+" nextItem.get('bottomEdge')="+nextItem.get('bottomEdge') : 'nextItem=false'));
      ...
      if(item.isChangingHeight === false && item.activeDropTarget === true && (draggedTopEdge - adjustment) < item.get('topEdge') && this.hasChild(item, 'activeDropTarget', '===', true) === 0)
      {
        ...
        if (item.swapFromFolder === false && item._height === item._originalHeight && item === this.currentlyDraggedComponent.get('parent'))
        {
          ...
          console.log("CHANGE: 4 shrinking manual");

          //manually set that we are changing drop target, so rest of items behave appropriately
          this.swapDropTarget = true;
          ...
          if(item._height !== parseFloat($(item.element).css('height')))
          {
            ...
            item.changeHeight(item._height, false); //don't run check
            //$(item.element).css({
      ...
      if($(this.currentlyDraggedComponent.ghostElement()).outerHeight() <= $(item.element).outerHeight())
      {
         ...
         */
         ...
         if(draggedTopEdge > item.get('topEdge'))
         {
          ...
          {
            console.log("others:1 for ="+item.elementId);
            return true;
          }
          ...
          if( prevItem && draggedBottomEdge > prevItem.get('bottomEdge')
            ...
            console.log("others:2 for ="+item.elementId);
            return true;
          }
...
/*
  ...
  updateEachSortItem(item, position, index, sortedItems) {
    ...
    //also ensure that we are not dragging from a nested folder into a parent folder and changing it's height. We don't need to. via hasChild
    if (this.swapDropTarget === true && item.activeDropTarget === true && item.swapFromFolder === false && this.hasChild(item, 'isBusy', '===', true) === 0)
    {
      //increase the droptarget height by the height of the dragged component, which includes the margin.
      //need to account for margin differences here
      //plus adjust for margin difference between folders and regular items
      console.log("doing this one="+item.elementId+"  "+this.hasChild(item, 'isDragging', '===', true))
      item._height = this.currentlyDraggedComponent.get('swappedDestinationHeight') + item._originalHeight;
    ...
    if(item._height !== item._originalHeight && item.swapFromFolder === false)
    {
      ...
      if((item.activeDropTarget === false || this.activeDropTargetComponent !== item) && this.hasChild(item, 'activeDropTarget', '===', true) === 0)
      {
      ...
      if(item._height !== parseFloat($(item.element).css('height')))
      {
          console.log("ADJUST: 2 change size");
     ...
     */
     ...
     if (!get(item, 'isDragging') && this.currentlyDropping === false) {
        ...
        if(this.insertedDragSpacer === false && this.makeSpacerForDraggedObject(item, position, index, sortedItems, itemParent, dimension))
        {
          //once the drag spacer is found on first time, stop checking for others.
          this.insertedDragSpacer = true;




          ///We should directly update the swappedDestinationHeight based on the item.
          //because this item is receiving the spacer above.
          ...
          //plus adjust for margin difference between folders and regular items
          //only if the drop target isn't our original
          console.log("MARGIN::0 this.swapDropTarget="+this.swapDropTarget+" item=drop?"+(this.dropTarget === item ? true: false )+" swappedDestinationHeight="+this.currentlyDraggedComponent.get('swappedDestinationHeight'));
          if(this.swapDropTarget === true && this.dropTarget !== item && this.currentlyDraggedComponent.get('swappedDestinationHeight') !== 0)
          {
            console.log("MARGIN::1 using swappedDestinationHeight="+item.elementId);
            position += this.currentlyDraggedComponent.get('swappedDestinationHeight');
          } else {
            console.log("MARGIN::2 not using swappedDestinationHeight="+item.elementId);
            //increase the position by the height of the dragged component, which includes the margin.
...
*/
      ...
      console.log("DIMENSION: A (SPACER) = item="+item.elementId+"get(item, dimension)="+get(item, dimension)+" _height="+(get(item, '_height') + parseFloat($(item.element).css('margin-bottom'))));
          ...
          //console.log("regular="+item.elementId);
                                        //Small fix inserted here. For drag-into then exit folder. When it shrinks, we haven't correctly indentified to insert drag spacer via makeSpacerForDraggedObject, so we do it here.
                                        var prevItem = this.findNearestItemNotDraggedScope(sortedItems, index, 'prev');
                                        if(prevItem.isChangingHeight === true && prevItem._height === prevItem._originalHeight)
                                        {
      ...
      set(item, direction, position);
      set(item, 'hasDragSpacerAbove', false);
      console.log("DIMENSION: C = get(item, dimension)="+get(item, dimension)+" _height="+(get(item, '_height') + parseFloat($(item.element).css('margin-bottom'))));
